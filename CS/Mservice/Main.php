...
class CS_Mservice_Main
{
    ...
    private static $init_max_attendees = 1000000;
...
//			
	...
	public function checkFBUserAndGetInfo1($first_name, $last_name, $about_me, $facebook_id, $email, $gender, $username, $dob, $FBaccessToken="",$device_id=0,$imei=0,$type="",$accesstoken="", $oncam_user_id = 0, $signstatus="signin")
    {
		$username = str_replace(".","",$username);
		$m_email = "";
		$m_dob = "";
...
public function uploadCustomerCanvasImageAndroid($user_id=0,$data=null){
	...
	public function getliveroombyuser_id($user_id, $event_id =0){
		if ($user_id>0){
			...
			$select ="select * from $table where profile_id=".$host_id." and event_id=".$event_id." and status = 1";
	...
	public function getLiveEvents($user_id,$nextRecordType="",$startFrom=0,$limit=15,$page=1) {       
		$cat=3; 
        ...
        $from = ($page-1)*15;        
        $widget = $resource->getTableName('widget_info');
        $sql = "select widget_id, widget_key, widget_title, user_id, username, created_on, invitee, call_status, is_private, hashtag from $widget where user_id IN(".$flwngs.") and call_status = 1 and is_private = 1 and invitee='p' ";
        
        $live_rooms = $read->fetchAll($sql);
        $sql.= " limit ".$from.", ".$limit;
        $live_room_limit = $read->fetchAll($sql);
                
        $room_pages =  ceil(count($live_rooms)/ 15);
		...
		$visibility = array(	
			...
			if($lastPage >= $page){
                ...
                foreach ($live_room_limit as $k =>$live_room){
                    $customer = Mage::getModel('customer/customer')->load($live_room['user_id']);        
                    $fc = strtolower(substr($customer->getFirstname(),0,1));
                    if($fc == ''){ $fc = rand(1,10); }
                    if((in_array($event['user_id'],$str)) && ($isLive == "true")){
                        $myFollowersroom = 1;
                    ...
                    //get paticipants
                    $table = $resource->getTableName('user_activities');
                    $sqlSelect = " Select profile_id, user_id, type_of, group_of, site, photo , created_on, status, id, webcam_on, mesg from $table where status > 0 and webcam_on =1 and profile_id =".$live_room['user_id']." and type_of = 'check-ins'";                                       
                    $activities = $read->fetchAll($sqlSelect);
                    $paticipants = count($activities);
                    
                    $result[$counter]['liveroom'] = array(
                        'room_id' => $live_room['widget_id'],
                        'room_title' => $live_room['widget_title'],
                        'room_name' => $live_room['widget_name'],
                        'host_id' => $live_room['user_id'],
                        'host_username' => $live_room['username'],
                        'firstname' =>  $customer->getFirstname(),
                        'lastname' =>  $customer->getLastname(),
                        'follower_room' =>$myFollowersroom,
                        'hashtag' => $live_room['hashtag'],
...
if ($client->getAccessToken()) {
  ...
  return "You are not a authenticated user";
   ...
   public function getFavouritePeopleAndFollowing($user_id=0,$page=1){
	...
	public function getVideoGrid($user_id, $limit=5, $privacy =0, $startFrom=0,$page=1){
		$resource = Mage::getSingleton('core/resource');
		$read= $resource->getConnection('core_read');
		$videoTable = $resource->getTableName('video');
	...
	if($user_id > 0){
		...
		*/
		//Get Block status				
		$select = "select * from cs_block_user WHERE user_id=".$current_user_id." and block_user_id=".$user_id." and status=1";
		$results = $read->fetchAll($select);
		if(count($results)>0){		
			$result['block']=true;
		}else{
	...
	public function getReplayVideosHome($user_id){
		...
		$read= $resource->getConnection('core_read');
		$videoTable = $resource->getTableName('video');
		$select = 'select video_id, title, identifier, description, profile_id, user_id, video_path, view_count, thumbnail_path, duration, tags, created_time  from '.$videoTable.' where isdeleted = 0 and status = 1 and show_home=1 order by video_id desc limit 0,15';
		$results = $read->fetchAll($select);
	...
	public function getLiveTopHashtag($user_id){
		...
		//==================================================================================================		
        $sqlSelect = " Select count( DISTINCT user_id) as total, profile_id from cs_user_activities where status > 0 and webcam_on =1 and type_of = 'check-ins'";                                       
        $sqlSelect.=" group by profile_id ORDER BY `total`  DESC limit 10";
        $activities = $read->fetchAll($sqlSelect);        
		$c=0;
        foreach($activities as $k=>$act){
			$str1[$c]=$act['profile_id'];
			$c = $c+1;
		}		
		$profile_ids = implode(',', $str1);
        
		$sqlSelect = " Select hashtag from cs_widget_info where user_id IN(".$profile_ids.") and invitee='p' and call_status=1 limit 10";                                               
        $livecalls = $read->fetchAll($sqlSelect);        
        if(count($livecalls)>0){        
			foreach($livecalls as $k=>$lcall){ 
                if($lcall['hashtag']!=''){
                    $counter++;            
                    $items[$counter] = $lcall['hashtag'];    
                }                                   
			}
		}
	...
	public function setHashtagBycallId($call_id, $hashtag){
		try{
			$resource = Mage::getSingleton('core/resource');
			$read= $resource->getConnection('core_read');
			$write = $resource->getConnection('core_write');
			$sql = "update cs_widget_info set hashtag='".$hashtag."' where widget_id=$call_id";
			$write->query($sql);
		}catch(Exception $e){
			return "Error: ".$e->getMessage();
		}
