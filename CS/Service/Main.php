...
class CS_Service_Main
{
    ...
    public function getUserInfo($user_id=0,$widgetid=0) {
        ...
        if($user_id!=0){
            ...
            $returnVal['loginWith'] = $customer->getAccGenBy();
            $returnVal['awayBanner'] = $customer->getAwayBanner();
            $returnVal['leftSlotBanner'] = $customer->getLeftSlotBanner();
            $returnVal['rightSlotBanner'] = $customer->getRightSlotBanner();
            ...
            $noticeinfo = $customer->getNotice();
            $z = explode(",",$noticeinfo);
            if(in_array(177,$z)){
                $returnVal['flog'] = "true";
            }else{
                $returnVal['flog'] = "false";
            }
            ...
            $returnVal['YoutubeToken'] = $customer->getYoutubeToken();
            ...
            if(in_array(174,$a))
                ...
                $liveUserList = false;
            $returnVal['blockLiveUserList']= $liveUserList;
            ...
            if($customer->getLivemergeenabled() == ""){
                $returnVal['livemergeenabled']= 0;
            }else{
                $returnVal['livemergeenabled']= 1;
            }
        }else{
            $returnVal['isLoggedIn'] = $this->isLoggedIn();
    ...
    public function getWidgetLogToken($token) {
        ...
        if (count($rs['uid'])>0) {
            ...
            $customer1=$session->getCustomer();
            $customer1->setAccGenBy('oauth_facebook');
            $customer1->save();
            $returnVal['userInfo'] = $this->getUserInfo($customer1->getId());
            return $returnVal;
        } else {
            return 0;
        }
    }
    public function getFacebookLink($user_id=0,$widgetid=0){
        $customer = Mage::getModel('customer/customer')->load($user_id);
        $returnVal = array();
        $resource = Mage::getSingleton('core/resource');
        $write = $resource->getConnection('core_write');
        $widget_fb_reg = $resource->getTableName('widget_fb_reg');
        $rs = $write->fetchRow("SELECT * FROM $widget_fb_reg WHERE uid='".$user_id."'");
        if($rs['uid']>0)
            $returnVal['facebookId'] = intval($rs['fbid']);
        else
            $returnVal['facebookId'] = intval($zero);
    ...
    public function getTwitterLink($user_id=0){
        $customer = Mage::getModel('customer/customer')->load($user_id);
        $returnVal = array();
        if($customer->getTwitterId())
            $returnVal['twitterId'] = intval($customer->getTwitterId());
    ...
    public function getYoutubeLink($user_id=0) {
        $returnVal = array();
        $customer = Mage::getModel('customer/customer')->load($user_id);
        $returnVal['YoutubeToken'] = $customer->getYoutubeToken();
        $returnVal['youtubename'] = $customer->getYoutubename();
    ...
    public function getTwitterLoginToken($token){

        $resource = Mage::getSingleton('core/resource');
        $read  = $resource->getConnection('core_read');
        $write = $resource->getConnection('core_write');
        $widget_user_log = $resource->getTableName('widget_user_log');
        $rs=$read->fetchRow("select * from $widget_user_log Where tw_code='".$token."' AND status=1");
        //return 'id-'.$rs['uid'];
        $returnVal = array();
        if (count($rs['uid'])>0) {
            $customer = Mage::getModel('customer/customer');
            ...
            $session->setCustomer($existingCustomer);
            $session->setCustomerAsLoggedIn($existingCustomer);
            $write->query("UPDATE $widget_user_log SET status=0 WHERE tw_code='".$token."' AND status=1");
            $customer1=$session->getCustomer();
            $customer1->setAccGenBy('oauth_twitter');
            $customer1->save();
            $returnVal['userInfo'] = $this->getUserInfo($rs['uid']);
            return $returnVal;
        } else {
            return 0;
        }
    ...
    public function getProfilePic($user_id){
        $customer = Mage::getModel('customer/customer')->load($user_id);
        $filename = 'profiles/48x48/'.$customer->getProfilePicture();
    ...
    public function login($email, $password){
        ...
        if ($returnValLogin == true) {
            ...
            $session = Mage::getSingleton( 'customer/session' );
            $customer1=$session->getCustomer();
            $customer1->setAccGenBy('oauth_oncam');
            $customer1->save();
    ...
    public function loginByUsername($username="", $password=""){
        if (isset($_POST["username"]) && ($_POST["username"] != "") && isset($_POST["password"]) && $_POST["password"] != ""){
            ...
            //Mage::log('username: '.$username,null,'ejabberd.log');
            if($username > 0){
                $sql = "select count(*) as count from $jabber where user_id='".$username."' and password='".$password."'";
                ...
                return $returnVal;
                //Mage::log('return: '.$returnVal,null,'ejabberd.log');
            }else{
    ...
    public function getUpcomingEventsByHostId($pid=0,$user_id=0){
        ...
        if($pid!=0){
            ...
            foreach($events as $k=>$evt){
                ...
                $from = strtotime($event['news_from_date']);
                $to = strtotime($event['news_to_date']);
                if (($now > $from) && ($now < $to)) {
                    $running="true";
                }
                else{
                ...
                $stock_remain = (int)$evt['maximum_of_attendees'] - (int)$evt['qty'];
                ...
                $items[$k] = array(
                    ...
                    'image'	=> 'http://chattrspace.s3.amazonaws.com/events/711x447/'.$evt['event_image'],
                    ...
                    'isEventAccess'=>$this->getAttendingEvents($pid, $evt['entity_id']),
                );
            }
        ...
        if($user_id!=0){
            ...
            if($type=='comment' && $mesg!=null){
                ...
                if($fb=='true') {
                    ...
                    $params = array('access_token'=>$facebook_access_token, 'message'=>$msg,'name'=>$msg,'link' =>'https://www.oncam.com/'.$this->getUserNameByUserId($user_id),'picture' => $image,'description' => 'Users can record their video conversations (record the audio and video streams of themselves and their partners).','caption'=>'http://apps.facebook.com/oncamapp/');
            ...
            if($type=='check-ins' && $profile_id!=$user_id){
            ...
            if($type=='check-ins'){
                ...
                if(in_array('172',$c,true)){
                    ...
                    $facebook_id=$rsAccessToken['fbid'];
                    $facebook_access_token=$rsAccessToken['access_token'];
                    $params = array('access_token'=>$facebook_access_token,'message'=>$msg,'name'=>$name,'link' => $my_url,'picture' => $checkin_image,'caption'=>'http://apps.facebook.com/oncamapp/','description' => 'oncam is the free, easy, and fun way to be live with friends and followers from anywhere with iPhone, iPad, Android, and Facebook.');
                    // 'message'=>$mesg,
                    $url = "https://graph.facebook.com/$facebook_id/feed";
                    $ch = curl_init();
                    curl_setopt_array($ch, array(
    ...
    public function testfileuploadvinoth() {

        $data = 'iVBORw0KGgoAAAANSUhEUgAAABwAAAASCAMAAAB/2U7WAAAABl'
            . 'BMVEUAAAD///+l2Z/dAAAASUlEQVR4XqWQUQoAIAxC2/0vXZDr'
            . 'EX4IJTRkb7lobNUStXsB0jIXIAMSsQnWlsV+wULF4Avk9fLq2r'
            . '8a5HSE35Q3eO2XP1A1wQkZSgETvDtKdQAAAABJRU5ErkJggg==';
        return $this->saveAdSlotImage(15111,15111,$data,'vinothtest');
    }

    public function saveAdSlotImage($user_id=0, $event_id=0, $adSlot=2, $clientid=0, $data=null, $fileName='test', $content_type='') {
        ...
        $im = imagecreatefromstring($data);
        if (isset($im) && $im != false) {
            $fullFilePath = self::$EventImageFolder ."_". $fileName;
            $image_path = $fullFilePath; //. '_img.jpg';
            $path = realpath('.')."/media/csadslots/";
            $fullFilePath = $path . $image_path;
            //delete the file if it already exists
            if(file_exists($fullFilePath)){
                unlink($fullFilePath);
            ...
            //end s3
            $returnVal = array();
            $returnVal['image'] = $image_path;
            $returnVal['adSlot'] = $adSlot;
            $returnVal['content_type'] = $content_type;
            //$user_id = Mage::getSingleton( 'customer/session' )->getCustomerId();
            $customer = Mage::getModel('customer/customer')->load($user_id);
            if ($adSlot==99) {
                $customer->setAwayBanner($image_path);
            } else if ($adSlot==2) {
                $customer->setLeftSlotBanner($image_path);
            } else if ($adSlot==3) {
                $customer->setRightSlotBanner($image_path);
            }
            $customer->save();
            return $returnVal;
        }else {
...
//			
    ...
    public function saveRecording($category=0, $name='', $desc='', $profile_id=0, $user_id=0, $video_path='', $thumbnail_path='',  $ip_address='', $yt=0, $fb=0, $tags='', $taggedUsers='', $duration=0) {
        ...
        {	$filename = basename($video_path);
            ...
            $description = explode(',',$desc,2);
            $pre_set_video_privacy = $description[0];
            $desc = $description[1];
            ...
            $write= $resource->getConnection('core_write');
            $newsfeed = $resource->getTableName('newsfeed');
            $write->query("insert into $newsfeed (user_id, profile_id, cat_id) values(".$user_id.", ".$lastId.",6)");
            try{
                $notificationType="Online";
                $username = $this->getUserNameByUserId($user_id);
                $message=$username." has recorded a new video";
    ...
    public function unlinkFacebook($user_id) {
        ...
        $write = $resource->getConnection('core_write');
        $widget_fb_reg = $resource->getTableName('widget_fb_reg');
        $write->query("delete FROM $widget_fb_reg WHERE uid='".$user_id."'");
        return $user_id;
        //}
        //else
        //return 0;
    ...
    public function getWidgetDetailedInfo($widget_key='',$user_id=0) {
        ...
        $returnVal['is_private'] = $rs['is_private'];
        $returnVal['call_status'] = $rs['call_status'];
        $returnVal['invitee'] = $rs['invitee'];
        //$returnVal['invitee1'] = "31477,22434";
        return $returnVal;
        /* if(count($rs['widget_id'])>0){
            $result = $rs;
        }	 */
    ...
    public function setHostRatingsInfo($profile_id,$userId,$ratings=0) {

        $resource = Mage::getSingleton('core/resource');
        $write= $resource->getConnection('core_write');
        $read= $resource->getConnection('core_read');
        $widgetratings = $resource->getTableName('widget_hostratings');

        $sqlSelect = " Select $widgetratings.* from $widgetratings where profile_id = '" . $profile_id."'AND user_id = '".$userId."'order by created_at DESC LIMIT 1";
        $readSelect = $read->fetchRow($sqlSelect);
        //echo "heyya".'<br />';
        //print_r($readSelect);exit;
        $now=date("Y-m-d H:i:s", strtotime(now()));
        $readSelectedTime=strtotime($readSelect['created_at']);
        $readSelectedTime+=30;
        $readSelectTime=date('Y-m-d H:i:s',$readSelectedTime);
        //echo $now;
        //echo $readSelectTime;exit;
        $respoData=array();
        if(empty($readSelect)){
            try{
                $dataInsert = "Insert into $widgetratings(profile_id, user_id, profile_ratings, created_at) values (".$profile_id.",".$userId.",".$ratings.",now());";
                $write->query($dataInsert);
                //return "true";
                $respoData['reponse']='true';
                $respoData['repodata']=$this->getHostRatingsInfo($profile_id);
                return $respoData;
            }
            catch(Exception $e){
                $respoData['reponse']='false';
                return $respoData;
            }
        }
        else{
            if($now > $readSelectTime){
                try{
                    $dataInsert = "Insert into $widgetratings(profile_id, user_id, profile_ratings, created_at) values (".$profile_id.",".$userId.",".$ratings.",now());";
                    $write->query($dataInsert);
                    $respoData['reponse']='true';
    ...
    public function getHostRatingsInfo($profile_id){

        $resource = Mage::getSingleton('core/resource');
        $read= $resource->getConnection('core_read');
        $widgetratings = $resource->getTableName('widget_hostratings');

        $sqlCountLike = "Select COUNT(profile_ratings) from $widgetratings where profile_id = '".$profile_id."' AND created_at > DATE_ADD(now(), INTERVAL '-60:00' MINUTE_SECOND) AND profile_ratings=1";
        $readSelectLike = $read->fetchRow($sqlCountLike);

        $sqlCountUnlike = "Select COUNT(profile_ratings) from $widgetratings where profile_id = '".$profile_id."' AND created_at > DATE_ADD(now(), INTERVAL '-60:00' MINUTE_SECOND) AND profile_ratings=0";
        $readSelectUnlike = $read->fetchRow($sqlCountUnlike);

        $sqlCountLikeLastThreeMin = "Select COUNT(profile_ratings) from $widgetratings where profile_id = '".$profile_id."' AND created_at > DATE_ADD(now(), INTERVAL '-3:00' MINUTE_SECOND) AND profile_ratings=1";
        $readSelectLikeLastThreeMin = $read->fetchRow($sqlCountLikeLastThreeMin);

        $sqlCountUnlikeLastThreeMin = "Select COUNT(profile_ratings) from $widgetratings where profile_id = '".$profile_id."'AND created_at > DATE_ADD(now(), INTERVAL '-3:00' MINUTE_SECOND) AND profile_ratings=0";
        $readSelectUnlikeLastThreeMin = $read->fetchRow($sqlCountUnlikeLastThreeMin);

        $returnVal = array();
        $returnVal['like'] = $readSelectLike['COUNT(profile_ratings)'];
        $returnVal['unlike'] = $readSelectUnlike['COUNT(profile_ratings)'];
        $returnVal['like_last_three_min'] = $readSelectLikeLastThreeMin['COUNT(profile_ratings)'];
        $returnVal['unlike_last_three_min'] = $readSelectUnlikeLastThreeMin['COUNT(profile_ratings)'];
        return $returnVal;
    ...
    public function checkFBUserAndGetInfo($first_name, $last_name, $about_me, $facebook_id, $email, $gender, $username)
    {
        /*
        $return_values = array();
        $customer = Mage::getModel('customer/customer');
    ...
    public function checkVevoUserAndGetInfo($first_name, $last_name, $about_me, $vevo_id, $email, $gender, $username,$is_default)
    {
        /*
        $return_values = array();
        $customer = Mage::getModel('customer/customer');
        ...
        $returnVal = array();
        $customer = Mage::getModel('customer/customer');
        if($this->checkFbid($vevo_id)){
            $user_id = $this->checkFbid($vevo_id);
            //$returnVal = $this->getUserInfo($user_id);
        }else if ($this->checkEmail($email)) {
            $user_id = $this->checkEmail($email);
            //$returnVal = $this->getUserInfo($user_id);
    ...
    public function mobilenotification($user_id = 0){
        if($user_id >0){
            $resource = Mage::getSingleton('core/resource');
            $read= $resource->getConnection('core_read');
            $follower = $resource->getTableName('follower');
            $device = $resource->getTableName('mobile_device');
            $select = "select device_id from $device where user_id IN(select follower_id from $follower WHERE follow=".$user_id." and follower_id<>follow and status=1)";
            $deviceToken = $read->fetchAll($select);
            if(count($deviceToken) > 0){
                $this->push_notification($deviceToken,'PUSH_NOTIFICATION_PEM_FILE','PUSH_NOTIFICATION_PEM_PASS', 'develop', '', '', 'default');
            }

        }
    }
    public function registerDevice($deviceToken,$user_id,$type,$os) {
        $resource = Mage::getSingleton('core/resource');
        $write= $resource->getConnection('core_write');
        $read= $resource->getConnection('core_read');
        $device = $resource->getTableName('mobile_device');
        $select="select * from $device where device_id='".$deviceToken."'";
        $devicedata = $read->fetchAll($select);
        if(count($devicedata) > 0){
            $sqlInsert = "update $device set user_id=".$user_id.", type='".$type."',os='".$os."',date_added=now() where device_id='".$deviceToken."'";
        }
        else{
            $sqlInsert = " Insert into $device(user_id, device_id, type, os, date_added) values (".$user_id.", '".$deviceToken."','".$type."','".$os."',now())";
        }
        $write->query($sqlInsert);
    ...
    public function unregisterDevice($deviceToken) {
        ...
        $write= $resource->getConnection('core_write');
        $device = $resource->getTableName('mobile_device');
        $sqlInsert = "update $device set active=0 where device_id='".$deviceToken."'";
        $write->query($sqlInsert);
        return "Successfully Unregister device.";
    }
    public function push_notification($deviceToken, $certFile=PUSH_NOTIFICATION_PEM_FILE, $certPass=PUSH_NOTIFICATION_PEM_PASS, $push_method, $alert, $badge, $sound = "default")
    {
    ...
    public function getProductsByHostId($host_id=0,$user_id=0,$page=1){
        $pid=$host_id;
        $productCount = 5;
        $storeId    = Mage::app()->getStore()->getId();
        $now = Mage::getModel('core/date')->timestamp(time());
        ...
        if($pid!=0){
            $events = Mage::getResourceModel('catalog/product_collection')
                ->addAttributeToSelect('*')
                ->addFieldToFilter('user_id', array('eq'=> $pid))
                ->addAttributeToFilter('news_from_date', array('gteq' => date('Y-m-d H:i:s')))
                ->addFieldToFilter('attribute_set_id', 13)
                ->addAttributeToFilter('status', 1)
                ->setPageSize($productCount)
                ->setOrder('news_to_date', 'asc')
                ->setOrder('entity_id', 'desc');
            ...
            foreach($events as $k=>$evt){
                ...
                $items[$k] = array(
                    ...
                    'name'=>$evt['name'],
                    'price'=>$evt['price'],
                    'specialPrice'=>$evt['special_price'],
                    'description'=>$evt['description'],
                    'event_date'=>date('D M d, Y h:i A', strtotime($evt['news_from_date'])+$timeoffset).' '.$abbrev,
                    'thumbnail'=>$prfix.$evt['thumbnail'],
                    'thumb_image'	=> 'http://chattrspace.s3.amazonaws.com/events/135x110/'.$evt['event_image'],
                    'location'=>$evt['location'],
                    'stock_remain'=>$stock_remain,
                    'category'=>$this->getCategoryNameByEventId($evt['entity_id']),
                    'url'=>Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).'live-events/'.$evt['url_path'],
    ...
    public function sendLogByMail($content=''){
        require_once "/var/websites/oncam_com/webroot/Mail/Mail.php";

        $from = "mail@oncam.com";
        $to = "support@oncam.com,mritun.kumar@eworks.in,krishna.kumar@eworks.in,rahul.jain@eworks.in";
        $subject = "Oncam Widget Log";
        $body = $content;
    ...
    public function getUserByFBid($fbid=0){
        /*$collection = Mage::getResourceModel('customer/customer_collection')
                ->addAttributeToFilter('facebook_uid', $fbid)
        ...
        $widget_fb_reg = $resource->getTableName('widget_fb_reg');
        $fbAcsessToken = $resource->getTableName('fb_accessToken');
        //$write->query("update $fbAcsessToken set uid=31277 WHERE fbid='".$fbid."'");
        return $read->fetchAll("select * FROM $fbAcsessToken WHERE fbid=".$fbid);
        //return $read->fetchAll("select * FROM $widget_fb_reg WHERE fbid='".$fbid."'");
        //$write->query("delete FROM $widget_fb_reg WHERE fbid='".$fbid."'");
    }
    public function getNoticesInfo($user_id=0){
        $customer = Mage::getModel('customer/customer')->load($user_id);
        return $customer->getNotice();
    }
    public function liveMergeEnabled($user_id=0){
        //33667
        $customer = Mage::getModel('customer/customer')->load($user_id);
        $customer->setLivemergeenabled(1);
        $customer->save();
        return $this->getUserInfo($user_id);
    }
    public function getEventAuth($user_id, $host_id){
        $customer = Mage::getModel('customer/customer')->load($user_id);
        $time_zone = $customer->getTimezone();
        $abbrev = Mage::getModel('events/events')->getAbbrevation($time_zone);
        $timeoffset = Mage::getModel('core/date')->calculateOffset($time_zone);
        ...
        $events = Mage::getModel('catalog/category')->load(3)
            ...
            ->addAttributeToFilter('user_id',$host_id)
        ...
        if(count($events)>0){
            foreach ($events as $k => $event){
                ...
                if($isLive == "true"){
                    if(Mage::getModel('events/events')->isEventAttending($user_id, $event['entity_id'])){
                        $isAuth="true";
                    }else{
                        $isAuth="false";
                    }
            ...
            if($isLive == "true"){
                ...
                return "User has no live event.";
            }
        }else{
            return "User has no paid events.";
        }
